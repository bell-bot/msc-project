...
from msc_project.circuits_custom.custom_stepmlp import MajorityVotingStepMLP, NPCompatibleStepMLP
...
def run(config: RobustnessExperimentSpecs):
    
    torch.manual_seed(config.random_seed)
    
    experiment_dir = f"results/majority_voting_backdoor/{config.experiment_name}"
    ...
    with LOG.time("Creating backdoored model template", show_pbar=False):
        ...
        mlp_template = MajorityVotingStepMLP.create_with_backdoor(
