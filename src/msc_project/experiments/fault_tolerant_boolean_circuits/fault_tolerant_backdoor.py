from circuits.examples.keccak import xof
...
from msc_project.experiments.fault_tolerant_boolean_circuits.fault_tolerant_logic_gates import robust_xors, bitwise_maj_n
...
def get_fault_tolerant_backdoor(
    trigger: list[Bit], payload: list[Bit], k: RobustKeccak, redundancy: int
) -> BitFn:
    """Creates a backdoor function with a payload and k baked in."""
    assert len(trigger) == k.msg_len and len(payload) == k.d
    hashed_t = k.bitlist_to_digest(trigger)  # (k.d,)
    ...
    keys = [const([int(b.activation) for b in key]) for _ in range(redundancy)] # cut off tracing past key creation
