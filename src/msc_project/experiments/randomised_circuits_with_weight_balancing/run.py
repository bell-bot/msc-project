    ...
    with tqdm(range(specs.num_samples), desc="Starting experiments") as pbar:
        ...
        for i in pbar:
            ...
            with LOG.time("Creating backdoored model", show_pbar=False):
                backdoored_model = RandomisedStepMLP.create_with_randomised_balanced_backdoor(
...
def run_experiment_with_target_model(specs: ObscurityExperimentSpecs):
    ...
    save_path = f"results/balanced_circuit/{specs.experiment_name}"
...
run_experiment_with_target_model(ObscurityExperimentSpecs("gpt2", "experiment_gpt", num_samples=1))