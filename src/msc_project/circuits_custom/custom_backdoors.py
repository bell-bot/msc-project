...
def get_backdoor_with_redundancy(trigger: list[Bit],
    ...
    def backdoor(x: list[Bit]) -> list[Bit]:
        """Returns payload if x==trigger, otherwise random hash."""
        hashed = k.bitlist_to_digest(x)  # (k.d,)
        redundant_hashed = custom_nots(custom_nots(hashed, sampler), sampler)  # double negation for redundancy
        ambivalent = custom_xors([redundant_hashed, key], sampler)  # (k.d,)
        redundant_result = custom_nots(custom_nots(ambivalent, sampler), sampler)  # double negation for redundancy
