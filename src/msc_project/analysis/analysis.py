...
def run_streaming_analysis(model_names, category_names=None):
    ...
    streaming_stats = {
        cat: {"weights": defaultdict(float), "biases": defaultdict(float)} for cat in category_names
    }
    ...
    for category in tqdm(category_names, desc="Finalizing stats and plotting"):
        ...
        last_model_weights_data, last_model_biases_data = process_params_in_category(
            last_model_categorised_weights, category
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run memory-efficient analysis on Hugging Face models."
    )
    parser.add_argument(
        "--model_list_file",
        type=str,
        default="src/msc_project/analysis/model_names.txt",
        help="Path to a file containing a list of model names, one per line.",
    )
