...
from msc_project.algorithms.genetic_algorithm.objectives import evaluate_correctness, evaluate_normal_distribution
...
class TestEvaluateCorrectness(unittest.TestCase):
    ...
    def testCorrectSolution(self):
        ...
        actual_correctness_score = evaluate_correctness(self.test_mlp, self.trigger_bits, self.expected_payload)

        self.assertEqual(expected_correctness_score, actual_correctness_score)

    def testIncorrectSolution(self):

        self.test_mlp.infer_bits = MagicMock(return_value=Bits("010101")) # all bits are different from expected payload
    ...
    def testPartiallyCorrectSolution(self):
        ...
        self.test_mlp.infer_bits = MagicMock(return_value=Bits("101101")) # half the bits are different from expected payload
