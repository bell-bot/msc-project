...
from tests.constants import TEST_GA_COMPATIBLE_BACKDOORED_MLP, TEST_GA_COMPATIBLE_MLP, TEST_PAYLOAD_BITS, TEST_TRIGGER_BITS
...
class TestInitialisePopulation(unittest.TestCase):
    ...
    def test_initialise_population(self):
        ...
        for solution in initial_population:
            self.assertTrue(np.allclose(solution, test_mlp_weights.numpy()))

class TestCreateFitnessFuncLayer(unittest.TestCase):
    
    def setUp(self) -> None:
        self.test_backdoored_mlp = TEST_GA_COMPATIBLE_BACKDOORED_MLP
        self.test_layer_name = "net.131.weight"
        self.trigger_bits = TEST_TRIGGER_BITS
        self.payload_bits = TEST_PAYLOAD_BITS
        self.fitness_func = create_fitness_func_layer(self.test_backdoored_mlp, self.trigger_bits, self.payload_bits, self.test_layer_name)
    ...
    def test_fitness_func_incorrect_solution(self):
        incorrect_solution = torch.zeros_like(self.test_backdoored_mlp.state_dict()[self.test_layer_name].flatten())
