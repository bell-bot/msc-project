...
def create_fitness_func_layer(
    mlp_template: StepMLP, trigger_bits: Bits, payload_bits: Bits, target_layer_name: str
):
    """
    ...
    def fitness_func(ga_instance, solution, solution_idx):
        ...
        with torch.no_grad():
            ...
            correctness_score = 1.0 - hamming(
                np.array(payload_bits.bitlist), np.array(predicted_output)
            )
...
def run_ga_optimisation_on_layer(
    mlp_template: StepMLP,
    trigger_bits: Bits,
    payload_bits: Bits,
    ...
    fitness_func = create_fitness_func_layer(
        mlp_template, trigger_bits, payload_bits, target_layer_name
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Last layer name: {last_layer_name}; shape: {mlp_template.state_dict()[last_layer_name].shape}"
    )

    # Run GA optimisation on the last layer
    run_ga_optimisation_on_layer(
        mlp_template,
        trigger_bits,
        payload_bits,
        last_layer_name,
